...
ifndef DEVICE
	...
	@if [ ! -f "$(COMMON_DIR)/$(DEVICE)_input.rb" ]; then \
		echo "Error: Input file '$(COMMON_DIR)/$(DEVICE)_input.rb' not found"; \
		exit 1; \
	fi
	@echo "Applying theme: $(THEME) for device: $(DEVICE)"
	@sed -e '/#{input_code}/r $(COMMON_DIR)/$(DEVICE)_input.rb' -e '/#{input_code}/d' $(THEME_DIR)/$(THEME)_app.rb > $(APP_FILE)
